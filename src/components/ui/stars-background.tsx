  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      stars.forEach((star) => {
        ...
        if (!isDark) {
          starOpacity = Math.max(0.75, starOpacity); // Minimum 75% opacity in light mode
        }

        if (star.isGlowing) {
          ctx.shadowBlur = isDark ? 8 : 16;
          ctx.shadowColor = `rgba(${colorRgb}, ${starOpacity})`;
          ctx.fillStyle = `rgba(${colorRgb}, ${isDark ? Math.min(1, starOpacity + 0.3) : 1})`; // 100% opacity when glowing in light mode
        } else {
          ctx.shadowBlur = 0;
